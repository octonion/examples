...
var player2_p func(total int,ace bool,cards [10]int,dc int) (float64,float64)

func init() {

	player2_p = memoize.Memoize(func(total int,ace bool,cards [10]int,dc int) (float64,float64) {
		...
		for i := 0; i < 10; i++ {
			if cards[i]>0 {
				...
				if total+i+1 > 21 {
					...
					cards[i] += -1
					e_h,e_s := player2_p(total+i+1,(ace||i==0),cards,dc)
					if (e_h>=e_s) {
						e_hit += p*e_h
					} else {
						e_hit += p*e_s
	...
	}).(func(total int,ace bool,cards [10]int,dc int) (float64,float64))
}
...
func init() {
	...
	player_p = memoize.Memoize(func(total int,ace bool,ace2 bool,cards [10]int,dc int) (float64,float64) {
		...
		e_h2,e_s2 := player2_p(t2,ace2,cards,dc)

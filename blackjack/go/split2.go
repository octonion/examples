		...
		for i := 0; i < 10; i++ {
			if cards[i]>0 {
				...
				if total+i+1 > 21 {
					...
					r += 10*(16*decks-cards[9])
					t2 = r-(dc+1)-total
					/*fmt.Println(t2,total+i+1,false,cards,dc)*/
...
func main() {
	...
	var e float64 = 0.0
	for i := 9; i < 10; i++ {
		p1 := float64(deck[i])/float64(52*decks-3)
		deck[i] += -1
		for j := 1; j < 2; j++ {
			p2 := float64(deck[j])/float64(52*decks-4)
			deck[j] += -1
			e_h,e_s := player_p(10+i+1,(i==0),(j==0),deck,6-1)
			fmt.Println(i,j,e_h,e_s)
			if (e_h>=e_s) {
				e += p1*p2*e_h
			} else {
				e += p1*p2*e_s
